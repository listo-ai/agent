...
|---|---|---|
| `config.tags` slot on `sys.auth.user` nodes | **Shipped** | `crates/domain-auth/manifests/user.yaml` |
| `sys.auth.user` + `sys.auth.tenant` node kinds | **Shipped** | `crates/domain-auth/` |
| `GET /api/v1/users` with `tags.labels` + `tags.kv.*` query filtering | **Shipped** | `crates/transport-rest/src/users.rs` |
| `POST /api/v1/users/{id}/grants` wire shape + `bulk_action_id` threading | **Shipped** | `crates/transport-rest/src/users.rs` |
| Rust client `client.users()` | **Shipped** | `clients/rs/src/users.rs` |
| TS client `client.users` | **Shipped** | `clients/ts/src/domain/users.ts` |
| CLI `agent users list / grant` | **Shipped** | `crates/transport-cli/src/commands/users.rs` |
| Enforcement boundary — auth schema allowlist + compile-fail proof | **Shipped** | `crates/auth/src/schema.rs` |
| Studio user-management UI (fan-out progress, retry) | **Deferred** | Separate landing |
| Actual Zitadel role-grant HTTP call | **Deferred** | Separate landing (Zitadel provider) |
| Versioned slot-history endpoint (`GET /api/v1/slots/history`) | **Deferred** | Additive compliance extension |

### Enforcement boundary — tags must never enter the auth resolution path

The "tags are not auth" rule is enforced structurally, not just by convention. Two independent mechanisms are shipped and run in CI:

**1. Schema-level allowlist** (`crates/auth/src/schema.rs`)

`auth_resolution_query_schema()` returns a `QuerySchema` that contains only the JWT claim fields relevant to scope resolution (`sub`, `org_id`, `roles`, `scope`). `tags.*` is absent from the allowlist. Calling `query::validate` against this schema with any filter referencing `tags.*` returns `Err(unknown field)` before any permission logic runs — rejected at schema-construction time, not runtime.

The user-management query schema (`user_management_query_schema()` in `transport-rest/src/users.rs`) is a **separate** schema that does expose `tags.labels` and pattern `tags.kv.*`, for Studio list views only. The two schemas are never shared.

Unit tests in `crates/auth/src/schema.rs` assert:
- `tags.labels=contains=team/platform` → `Err("unknown field")` on the auth schema.
- `tags.kv.site==abc` → `Err("unknown field")` on the auth schema.
- `tags.*` filters succeed on the user-management schema (proving the user-facing path still works).

These tests run as part of `make test` / CI. A PR routing tags into the auth schema fails before review.

**2. Structural dependency exclusion + compile-fail doctest** (`crates/auth/src/schema.rs`)

`domain-tags` is intentionally absent from `crates/auth/Cargo.toml`. Any code that attempts to import it inside `auth/` fails to compile. This is encoded as a `compile_fail` doctest in `crates/auth/src/schema.rs`:
...
This doctest runs as part of `make test-doc` (included in `make ci`). If someone adds `domain-tags` as a dep of `crates/auth`, the doctest inverts and CI fails immediately.
...
**What's shipped (this PR):** `POST /api/v1/users/{id}/grants` accepts `{ role, bulk_action_id }` and returns `202 Accepted` with `{ user_id, role, bulk_action_id, status: "accepted" }`. The Rust client (`client.users().grant_role(…)`), TS client (`client.users.grantRole(…)`), and CLI (`agent users grant`) all wire the `bulk_action_id` field through. The Studio fan-out UI and Zitadel call are a separate landing.

**What's deferred:** Studio progress UI, per-grant retry UX, the actual Zitadel role-grant HTTP call. These land when Studio's user-management page ships.

### Compliance and versioned tag history *(deferred — additive extension)*
