...
|---|---|
| Rust | [/home/user/code/workspace/SKILLS/rust.md](../../../SKILLS/rust.md) |
| TypeScript / React | [/home/user/code/workspace/SKILLS/ts.md](../../../SKILLS/ts.md) |
| Go | [/home/user/code/workspace/SKILLS/golang.md](../../../SKILLS/golang.md) |
| Dart / Flutter | [/home/user/code/workspace/SKILLS/dart.md](../../../SKILLS/dart.md) |
...
No private entities. No parallel state. Devices, users, flows, alarms, dashboards, the agent's own health — **all nodes**. If you're about to write a subsystem that owns `Mutex<SomeState>` that nobody outside the subsystem can observe, **stop** — promote the state to a kind with status slots. See [EVERYTHING-AS-NODE.md](EVERYTHING-AS-NODE.md).
...
- **`ui-core` owns the "brain".** Every React hook that talks to the agent lives here. Studio imports from `ui-core`, not from `agent-client-ts` directly if a hook would wrap the call.
...
Not later. Same PR. See [TESTS.md](TESTS.md).

### Rule F — `Msg` is immutable on the wire

Node-RED parity. Produce new messages via `Msg::new` / `Msg::child`; don't mutate. The Rhai Function-node is the one place `msg` feels mutable to the author — the runtime snapshots it on exit. See [EVERYTHING-AS-NODE.md § "Wires, ports, and messages"](EVERYTHING-AS-NODE.md).

### Rule G — Layer separation

`transport → domain → data` within the agent. Never the other way. No SQL in handlers, no HTTP in domain. See [CODE-LAYOUT.md](CODE-LAYOUT.md).

### Rule H — Versioning on contract surfaces

Changes to `spi`, public client APIs, kind manifests, and `Msg` shape are versioned per [VERSIONING.md](VERSIONING.md). Add-only within a major.

### Rule I — Transport handlers stay thin

This is the rule most often forgotten under deadline pressure. **A REST/gRPC/CLI handler does four things**: (1) extract inputs, (2) call a domain function, (3) map the result to a DTO, (4) return. If your handler has business logic, containment rules, graph walks, or anything that would apply equally to a gRPC or NATS surface — **that logic doesn't belong in `transport-*`**. Move it to `graph`, `domain-*`, or a shared crate, and have every transport call the same function.
...
→ **[`agent/crates/domain-*`](../../crates)** — `domain-compute` for transforms, `domain-logic` for control flow, `domain-function` for scripting, or create a new sibling crate if the concern is new.

Then: register the kind in [`apps/agent/src/main.rs`](../../crates/apps/agent/src/main.rs) alongside the others — `kinds.register(<X as NodeKind>::manifest())` plus `behaviors.register(…)`. Read [NODE-AUTHORING.md](NODE-AUTHORING.md).
...
→ **[`agent/crates/transport-rest`](../../crates/transport-rest)** for the route.
...
For filter/sort/pagination surfaces, use the generic [RSQL query framework](QUERY-LANG.md) — add a `QuerySchema` declaratively, don't hand-roll a parser. One schema, every transport (REST, CLI, MCP, NATS) gets the filter surface for free.
...
**MUST NOT** `import` from `@listo/ui-core` directly from a block. `block-ui-sdk` is a curated re-export facade — if something you need isn't there, **add it to `block-ui-sdk`** first. Direct ui-core imports are a layering violation every time.

### Q9. Is this the `agent <cmd>` CLI (built into the agent binary)?

*Examples: `agent kinds list`, `agent slots write`, `agent flows edit`.*

→ **[`agent/crates/transport-cli`](../../crates/transport-cli)** — thin clap wrapper over `agent-client-rs`.
...
→ **[`agent-cli`](../../../agent-cli)** for the MCP bootstrap, and **[`agent/crates/transport-mcp`](../../crates/transport-mcp)** for the tool surface.
...
→ **[`repos-cli`](../../../repos-cli)** for the tool itself; **[`workspace/mani.yaml`](../../../mani.yaml)** for task/project definitions.

### Q14. Is this documentation?

- **Design spec / architecture** → `agent/docs/design/` (where this doc lives).
...
→ Read [OVERVIEW.md](OVERVIEW.md) for the full repo map + dependency arrow, then ask the user.

---

## 3 — What each library is for

Cheat-sheet summary. Full map: [OVERVIEW.md § "The workspace"](OVERVIEW.md#the-workspace--repos-roles-and-ownership-boundaries).
...
| [`agent`](../../) | — (private) | The platform: engine, graph, blocks-host, domain-*, transport-*, the binary. |
...
Before merging, ask: **if someone deletes `/home/user/code/workspace/studio` entirely and only has `ui-kit` + `ui-core` + `agent-client-ts` on npm, can they build a working admin UI?**
...
mani run dev        --projects agent   # cloud :8081 + edge :8082 + both Studios
...
The full task list is in [`workspace/mani.yaml`](../../../mani.yaml) — `mani list tasks` to see it.
...
| Task | Read, in order |
|---|---|
| Run local tests / start dev env | This doc → [TESTING.md](../testing/TESTING.md) |
| Add a new built-in node kind | This doc → [EVERYTHING-AS-NODE.md](EVERYTHING-AS-NODE.md) → [NODE-AUTHORING.md](NODE-AUTHORING.md) → [CODE-LAYOUT.md](CODE-LAYOUT.md) |
| Build a new block | This doc → [EVERYTHING-AS-NODE.md](EVERYTHING-AS-NODE.md) → [NODE-AUTHORING.md](NODE-AUTHORING.md) → [BLOCKS.md](BLOCKS.md) → [VERSIONING.md](VERSIONING.md) |
| Add / modify a REST endpoint | This doc → [QUERY-LANG.md](QUERY-LANG.md) → [EVERYTHING-AS-NODE.md](EVERYTHING-AS-NODE.md) → [AUTH.md](AUTH.md) → [CODE-LAYOUT.md](CODE-LAYOUT.md) |
| Work on the Studio or alt frontend | This doc → [UI.md](UI.md) → [NODE-RED-MODEL.md](NODE-RED-MODEL.md) (msg shape the UI renders) |
| Work on `graph` / `engine` | This doc → [EVERYTHING-AS-NODE.md](EVERYTHING-AS-NODE.md) (entire) → [RUNTIME.md](RUNTIME.md) → [CODE-LAYOUT.md](CODE-LAYOUT.md) |
| Auth / Zitadel / JWT | This doc → [AUTH.md](AUTH.md) |
| Messaging / NATS / subjects | This doc → [EVERYTHING-AS-NODE.md § event model](EVERYTHING-AS-NODE.md) → [RUNTIME.md § outbox](RUNTIME.md) |
| Persistence / database | This doc → [CODE-LAYOUT.md § data-* crates](CODE-LAYOUT.md) → [EVERYTHING-AS-NODE.md § persistence](EVERYTHING-AS-NODE.md) |
| Time-series / telemetry | This doc → [QUERY-LANG.md § Time-series](QUERY-LANG.md#time-series-query-shape-the-non-rsql-companion) |
| MCP | This doc → [MCP.md](MCP.md) → [AUTH.md](AUTH.md) |
| CLI | This doc → [CODE-LAYOUT.md](CODE-LAYOUT.md) → [QUERY-LANG.md](QUERY-LANG.md) |
| Versioning / capabilities | This doc → [VERSIONING.md](VERSIONING.md) |
| Logging / tracing | This doc → [LOGGING.md](LOGGING.md) |
| Writing tests | This doc → [TESTS.md](TESTS.md) |
| Cross-cutting refactor / unsure | This doc → [OVERVIEW.md](OVERVIEW.md) → [CODE-LAYOUT.md](CODE-LAYOUT.md) → **ask the user** |
...
1. Q4 (REST) → need a filter surface on `/api/v1/kinds`. → `agent/crates/transport-rest/src/kinds.rs`. Add a `QuerySchema` per [QUERY-LANG.md](QUERY-LANG.md), add `org` as a derived `KindDto` field.
...
**The fix.** Extracted [`graph::placement_allowed(parent_kind, parent_manifest, candidate) -> bool`](../../crates/graph/src/placement.rs) — one pure function with its own tests. Both `GraphStore::create_child` (5 lines: "if `!placement_allowed(..)` → `PlacementRejected`") and `transport-rest/src/kinds.rs` (one line: `.retain(|c| placement_allowed(...))`) call it. Missing `isAnywhere` carve-out now fixed in one place, not two.
