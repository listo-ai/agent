...
fn on_init_seeds_count_from_initial() {
    ...
    behaviors
        .set_config(id, json!({ "initial": 7, "step": 1 }))
        .unwrap();
...
fn on_message_increments_and_emits() {
    ...
    let id = graph.get(&path).unwrap().id;
    behaviors
        .set_config(id, json!({ "initial": 0, "step": 1 }))
        .unwrap();
    behaviors.dispatch_init(id).unwrap();
    sink.take();
...
fn slot_source_regression_external_write_wins() {
    ...
    let id = graph.get(&path).unwrap().id;
    behaviors
        .set_config(id, json!({ "initial": 10, "step": 1 }))
        .unwrap();
    behaviors.dispatch_init(id).unwrap();
    sink.take();
...
fn reset_via_port_returns_to_initial() {
    ...
    let id = graph.get(&path).unwrap().id;
    behaviors
        .set_config(id, json!({ "initial": 100, "step": 1 }))
        .unwrap();
    behaviors.dispatch_init(id).unwrap();
    sink.take();
...
fn reset_via_msg_metadata_returns_to_initial() {
    ...
    let id = graph.get(&path).unwrap().id;
    behaviors
        .set_config(id, json!({ "initial": 50, "step": 1 }))
        .unwrap();
    behaviors.dispatch_init(id).unwrap();
    sink.take();
